...
donotplunge = CheckBox(  app,                       text="Dry fire (do not plunge)?",   grid=[0,10,2,1], align='left')
button_up   = PushButton(app, command=powerup,      text="        Ready        ", grid=[0,11])
button_pulse= PushButton(app, command=pulsestartprocess, text="Pulse & Plunge",  grid=[1,11])
button_start= PushButton(app, command=startprocess, text="Spray & Plunge",  grid=[0,12])
button_down = PushButton(app, command=powerdown,    text="         Abort         ",   grid=[1,12], align='left')
